...
- 点击“查看”或在列表按 Space 打开完整文本、HTML/RTF 的纯文本表示、图片或文件路径；文本和路径可滚动、选取，图片按原比例显示；复制按钮随记录类型切换。文本、网址与富文本可从预览进入编辑，保存后保留记录 ID、分组与收藏状态并刷新搜索结果；修改富文本会转为纯文本，空内容保存会报错，请使用独立“删除”按钮。Esc 在编辑时取消修改，在只读预览时返回列表。预览按需从后台读取，关闭后的旧响应不会重新打开预览。
...
- 每条记录可从卡片正文直接拖动，落在目标上/下半区即插到前/后；按钮区保留各自点击操作，边缘拖动滚动已加载列表。顺序写入 SQLite，收藏列表维护独立顺序。置顶与普通记录分别排序，跨区拖动会提示先调整置顶状态。Esc 可取消拖动。
...
- 主窗口“导出备份”保存版本化 GPUI ZIP：在线 SQLite 快照、引用的可读取图片及格式说明。默认排除非 GPUI 设置（包括旧库潜在的密码或 token）；源图片缺失时显示数量，记录保留原路径。备份不包含普通文件记录指向的原文件。恢复使用 `elegant-clipboard-gpui --import-backup <备份.zip> --data-dir <空目录>`，验证格式、数据库与图片引用后才安装，不覆盖已有数据库或图片。恢复期间目标目录不能由其他实例使用。此命令不接受旧版 Tauri ZIP；旧库仍用 `--import-db`。
...
当前 139 项测试通过（核心 115、Windows 后端 20、应用状态/参数 4），覆盖暂停确认与重启恢复、搜索代次与选中 ID 保持、自定义分组浏览、创建、重命名、安全删除、记录移动与组内排序、文本编辑与过期内容拒绝、HTML/RTF 二进制存储与混排拖动、图片存储、文本/图片/文件路径预览、容量淘汰清理、文件源缺失、键盘边界、冒烟参数隔离，以及在线旧库导入、GPUI 备份导出恢复、网址兼容、拒绝无效来源、实例唤出和快捷键设置。fmt、Clippy（warnings 视为错误）、Windows debug/release 构建及 release 窗口冒烟通过；分组栏、整卡拖动和最小尺寸布局已在隔离窗口验收。
