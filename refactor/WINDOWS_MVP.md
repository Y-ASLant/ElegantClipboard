...
- 导入的自定义分组会在筛选栏下方显示。点击分组切换其历史，搜索、收藏筛选、数量与分页均限定在当前分组；分组栏在窄窗口中横向滚动。当前仍只向默认分组采集新内容，创建/重命名/删除分组和移动条目尚未接入。
...
当前 121 项测试通过（核心 104、Windows 后端 13、应用状态/参数 4），覆盖暂停确认与重启恢复、搜索代次与选中 ID 保持、自定义分组浏览与组内排序、HTML/RTF 二进制存储与混排拖动、图片存储、文本/图片/文件路径预览、容量淘汰清理、文件源缺失、键盘边界、冒烟参数隔离，以及在线旧库导入、网址兼容、拒绝无效来源、实例唤出和快捷键设置。fmt、Clippy（warnings 视为错误）和 Windows debug/release 构建及 release 窗口冒烟通过；新增分组栏已经完成 debug 窗口和最小尺寸验收。
