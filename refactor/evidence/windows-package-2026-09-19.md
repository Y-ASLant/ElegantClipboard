...
- 重新打包当前代码后，同名 ZIP 的 SHA-256 为 `729457db12e178bd3d3fc7d5dab57e001cf538bdefabe618e2ea43f0bd1334cb`，与 `.sha256` 相符。归档包含 exe、LICENSE 和 README.txt，包内 exe 的哈希与当前 Release 构建一致。解压到隔离目录 `target/qa/package-current-1724062e632747e1b524e90c29c7ed6d/` 后，从包内 exe 对已有的两条合成历史执行 `--import-db`，输出“导入完成：2 条记录”，再以导入目录运行 `--smoke-test`，退出码为 0。以上先前摘要只用于追溯，均已不对应当前同名 ZIP。
