...
GPUI 窗口创建失败时，启动入口现在退出应用事件循环并返回具体错误，而不再在 `expect` 处崩溃；已启动的后台服务随作用域释放。`cargo test --workspace --locked --quiet` 的 161 项测试、严格 Clippy、fmt、Windows Release 构建和隔离 `--smoke-test` 正常窗口启动均通过。未通过人为破坏图形环境来制造窗口创建失败，因此故障分支的系统级退出行为仍待专用环境验收。
