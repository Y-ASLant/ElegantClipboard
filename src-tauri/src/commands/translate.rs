...
fn translate_baidu(client: &reqwest::blocking::Client, text: &str, from: &str, to: &str, app_id: &str, secret_key: &str) -> Result<String, String> {
    ...
    fn map_lang(lang: &str) -> &str {
        match lang {
            "auto" => "auto", "zh" => "zh", "en" => "en",
            "ja" => "jp", "ko" => "kor", "fr" => "fra", "de" => "de",
            "es" => "spa", "pt" => "pt", "ru" => "ru", "ar" => "ara",
            "it" => "it", "th" => "th", "vi" => "vie", other => other,
        }
    }
    ...
    let params = [
        ("q", text), ("from", from_baidu), ("to", to_baidu),
    ...
    let results = val["trans_result"].as_array()
        .ok_or_else(|| "翻译结果格式异常".to_string())?;
    let translated: Vec<&str> = results.iter().filter_map(|r| r["dst"].as_str()).collect();
    if translated.is_empty() { Err("翻译结果为空".to_string()) } else { Ok(translated.join("\n")) }
}

/// OpenAI / AI 翻译
fn translate_openai(
    client: &reqwest::blocking::Client,
    text: &str, from: &str, to: &str,
    ...
    let model_id = if model.is_empty() { "gpt-4o-mini" } else { model };
...
pub async fn translate_text(
    ...
    proxy_mode: String, proxy_url: String,
    ...
    tokio::task::spawn_blocking(move || {
        ...
        match provider.as_str() {
            ...
            "deeplx" => translate_deeplx(&client, &text, &from, &to, &deeplx_endpoint.unwrap_or_default()),
            "google_free" => translate_google_free(&client, &text, &from, &to),
            "google_api" => translate_google_api(&client, &text, &from, &to, &google_api_key.unwrap_or_default()),
            "baidu" => translate_baidu(&client, &text, &from, &to, &baidu_app_id.unwrap_or_default(), &baidu_secret_key.unwrap_or_default()),
            "openai" => translate_openai(&client, &text, &from, &to, &openai_endpoint.unwrap_or_default(), &openai_api_key.unwrap_or_default(), &openai_model.unwrap_or_default()),
...
pub fn register_translate_selection_shortcut(app: &tauri::AppHandle) {
    ...
    let translate_enabled = settings_repo.get("translate_enabled").ok().flatten()
        .map(|v| v == "true").unwrap_or(false);
    if !translate_enabled { return; }

    let enabled = settings_repo.get("translate_selection_enabled").ok().flatten()
        .map(|v| v == "true").unwrap_or(false);
    if !enabled { return; }
...
pub async fn update_translate_selection_shortcut(
    ...
    let settings_repo = database::SettingsRepository::new(&state.db);
    settings_repo.set("translate_selection_shortcut", &new_shortcut)
